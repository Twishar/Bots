...
UKRAINIAN_POST_SHIPPING_OPTION = types.ShippingOption(
    id='ru_post', title='Почтой России')
UKRAINIAN_POST_SHIPPING_OPTION.add(
    types.LabeledPrice(
...
PICKUP_SHIPPING_OPTION.add(types.LabeledPrice('Самовывоз в Киеве', 50000))
...
async def process_shipping_query(shipping_query: types.ShippingQuery):
    ...
    if shipping_query.shipping_address.country_code == 'UK':
        shipping_options.append(UKRAINIAN_POST_SHIPPING_OPTION)

        if shipping_query.shipping_address.city == 'Киев':
